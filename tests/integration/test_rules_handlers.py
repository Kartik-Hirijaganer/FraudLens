...
+ not-found. The HTTP wiring (auth, envelope, cross-tenant routing) is covered separately in
test_rules_api.py."""
...
async def test_update_applies_fields_and_bumps_version(db_session: AsyncSession) -> None:
    ...
    updated = await update_rule(uuid.UUID(created.rule_id), update, tenant, db_session)
...
async def test_update_partial_only_enabled_leaves_other_fields(db_session: AsyncSession) -> None:
    ...
    updated = await update_rule(
        uuid.UUID(created.rule_id), RuleUpdateRequest(enabled=False), tenant, db_session
    )
    assert updated.enabled is False
...
async def test_update_missing_raises(db_session: AsyncSession) -> None:
    tenant = await _tenant(db_session)
    with pytest.raises(AppError) as excinfo:
        await update_rule(uuid.uuid4(), RuleUpdateRequest(enabled=False), tenant, db_session)
    assert excinfo.value.code == "rule_not_found"
...
async def test_delete_removes_and_missing_raises(db_session: AsyncSession) -> None:
    tenant = await _tenant(db_session)
    created = await create_rule(_create(), tenant, db_session)
    await delete_rule(uuid.UUID(created.rule_id), tenant, db_session)
    with pytest.raises(AppError) as excinfo:
