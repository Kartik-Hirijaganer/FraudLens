...
def test_the_deploy_is_manual_only() -> None:
...
def test_manual_dispatch_always_reaches_the_identity_and_ci_gates(job: str) -> None:
    assert "if" not in _job(job)


def test_a_dispatch_deploys_the_ref_it_was_dispatched_against() -> None:
    body = _source()
    assert "github.event.workflow_run" not in body
    assert body.count("${{ github.sha }}") >= 1
    assert "DEPLOY_SHA: ${{ github.sha }}" in body
...
def test_dispatching_does_not_weaken_any_gate() -> None:
    ...
    assert jobs["verify"]["with"] == {"scope": "backend"}
