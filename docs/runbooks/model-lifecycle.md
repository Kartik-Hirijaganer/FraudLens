...
> §10.5.1 promotion gates**; the human-gated promotion workflow (shadow → canary → active →
> rollback, drift) lands in Phase 10. See plan
> [§10.5 / §10.5.1](../../plans/2026-06-12-aml-fraud-detection-system.md) and
> [§9.2](../../plans/2026-06-12-aml-fraud-detection-system.md) (model-lifecycle tables).
...
| Promotion gates | [`scoring/gates.py`](../../packages/fraudlens-ml/src/fraudlens_ml/scoring/gates.py) | The §10.5.1 metrics + pass/fail logic, reused by training (Phase 5) and retrain (Phase 10). |
| Canary router | [`scoring/router.py`](../../packages/fraudlens-ml/src/fraudlens_ml/scoring/router.py) | Deterministic active-vs-canary split by routing key (stub in v1; wired in Phase 10). |
...
Per-tenant evaluation slices (no slice worse than active − `0.05` PR-AUC) and the canary
auto-abort rule are part of the same gate family and land with the Phase 10 retrain/eval flow
(§9.4). All thresholds are documented defaults and configurable via `system_config.modelGates`.
