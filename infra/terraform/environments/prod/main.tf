# Prod environment composition (plan §15.1) — a resource group wiring the modules into the v1
...
module "gateway_app" {
  ...
  storage_container_name         = module.blob.container_name
  sar_pdf_container_name         = module.blob.sar_pdf_container_name
  retrain_job_name               = "${var.name_prefix}-retrain"
  batch_score_job_name           = "${var.name_prefix}-batch-score"
