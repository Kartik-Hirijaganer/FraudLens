...
run repeatedly without creating duplicates. Phase 2 seeds the foundation the schema can hold
without later-phase code: the demo agency + its analyst/reviewer/admin users (shared with the
auth dev-bypass via fraudlens_backend.demo), the default global `system_config` tunables, and
the **active fixture model** (a training dataset → training run → ACTIVE model version → the
single deployment pointer). The run itself is recorded in `job_executions`. Transactions,
rules, RAG, runs, and real model artifacts are seeded by their own phases, which extend this
script. Refuses to run when `environment == "prod"`.

Key classes:
- SeedSummary: counts of the demo entities the seed ensured exist.

Key functions:
- seed: idempotently upsert the Phase 2 demo dataset into a session (no commit).
...
- The fixture model carries only feature NAMES + a content hash — no PHI, no agency_id as a
  feature (ADR-015); the real trained artifact lands in Phase 5.
"""
...
_FIXTURE_FEATURE_SPEC: dict[str, Any] = {
    "version": 1,
    "features": ["amount", "hour_of_day", "country_risk", "velocity_24h", "round_amount_flag"],
}

# Default GLOBAL (agency_id NULL) runtime tunables (plan §9.1 `system_config` keys).
_DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "labelMaturityDays": 7,
    "canaryPercent": 0,
    "modelGates": {"prAucFloor": 0.5, "maxRegression": 0.02},
    ...
    if await session.get(ModelTrainingRun, _FIXTURE_TRAINING_RUN_ID) is None:
        session.add(
            ModelTrainingRun(
                ...
                params={"fixture": True},
                metrics={"pr_auc": 0.5, "note": "fixture model — real training in Phase 5"},
                artifact_uri="fixtures/models/v0-fixture.json",
            )
        )
    if await session.get(ModelVersion, _FIXTURE_MODEL_VERSION_ID) is None:
        session.add(
            ModelVersion(
                ...
                notes="Seed fixture model version (Phase 2); real artifact in Phase 5.",
            )
        )
...
async def _amain() -> int:
    ...
    print(
        ...
        f"{summary.model_versions} active fixture model"
    )
    return 0
