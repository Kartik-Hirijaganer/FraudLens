...
5. `/readyz`: replace `_openrouter` with `_llm_provider` (provider = SAR config model prefix; URL from `resolve_base_url`; check name `llmProvider`, `detail=<provider>`). **Defect fix:** the `infisical` check is required only when a probe is registered; otherwise `skipped` and excluded from the live-required set — regression test: a live-mode app with database/chromadb/supabaseAuth/llmProvider `ok` and no Infisical probe returns 200. Only `tests/integration/test_ops.py` and one runbook line reference the old `openrouter` name.
...
| vLLM route + readiness fix | `pytest tests/integration/test_sar_drafter_vllm_route.py`; live-mode `/readyz` 200 without an Infisical probe | 4 |
